...
export const login = async(
    ...
    if(response.data.success && response.data.data)
    {
        ...
        setAuthTokens(tokens.accesToken,tokens.refreshToken)
    }
...
export const refreshAuth = async (): Promise<APIResponse<AuthTokens>> => {
  ...
  if (response.success && response.data) {
    setAuthTokens(response.data.accesToken, response.data.refreshToken);
  } else {
...
const verifyTokenAndGetUserWithRefresh = async (): Promise<APIResponse<User>> => {
  ...
  if (!verifyResponse.success) {
    ...
    if (refreshResult.success && refreshResult.data) {
      ...
      setAuthTokens(refreshResult.data.accesToken,refreshResult.data.refreshToken)
    } else {
      return {
        success: false,
